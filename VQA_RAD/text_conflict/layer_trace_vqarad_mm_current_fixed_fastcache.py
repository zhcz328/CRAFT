...
def is_qwen_full_forward_model(model) -> bool:
    candidates = [model]
    ...
    for cand in candidates:
        ...
        if getattr(cfg, "model_type", "") in {"qwen3_5", "qwen3_vl"}:
            return True
    return False
...
def build_mm_messages(prompt: str, image: Image.Image):
    return [
        {
            "role": "system",
...
def maybe_collect_cuda_garbage(step_idx: int, every: int = 1) -> None:
...
def build_shard_prefix_path(path_value: str, shard_idx: int, num_shards: int) -> str:
    path = Path(path_value)
    shard_dir = path.parent / f"{path.name}_shards"
    shard_dir.mkdir(parents=True, exist_ok=True)
    return str(shard_dir / f"{path.name}.{build_shard_token(shard_idx, num_shards)}")


def iter_rows_for_shard(rows, num_shards: int, shard_idx: int):
    for row_idx, row in enumerate(rows):
        if row_idx % num_shards == shard_idx:
            yield row_idx, row
...
def build_scan_plan(
    ...
    # 4) de-duplicate layers across rounds in order.
    used = set()
...
def build_inputs_mm(processor, prompt: str, image: Image.Image, device):
    msgs = build_mm_messages(prompt, image)
    base_inputs = build_chat_template_inputs(processor, msgs)
...
@torch.inference_mode()
def build_qwen_full_inputs(processor, prompt: str, image: Image.Image, continuation: str):
    msgs = build_mm_messages(prompt, image)
    ...
    return build_processor_inputs_mm(processor, full_text, image)
...
class PromptPrefixPatcher:
    def __init__(
        ...
        self.patch_all_tokens = patch_all_tokens
        self.handle = None
    ...
    def __enter__(self):
        ...
        def hook(module, inp, out):
            ...
            hs = hs.clone()
            prompt_len = min(self.prompt_len, hs.shape[1])
            if prompt_len <= 0:
                return out

            if self.patch_all_tokens:
                kk = min(prompt_len, self.source_tail.shape[1])
                hs[:, :kk, :] = self.source_tail[:, :kk, :].to(hs.device, hs.dtype)
            else:
                kk = min(self.patch_k, prompt_len, self.source_tail.shape[1])
                if kk <= 0:
                    return out
                hs[:, prompt_len - kk:prompt_len, :] = self.source_tail[:, -kk:, :].to(hs.device, hs.dtype)
...
def logprob_qwen_full_forward(
    ...
    source_tail: Optional[torch.Tensor] = None,
    patch_k: int = 0,
    patch_all_tokens: bool = False,
) -> float:
    prompt_inputs = build_inputs_mm(processor, prompt, image, model.device)
    full_inputs = build_qwen_full_inputs(processor, prompt, image, continuation)
    ...
    if patch_layer_idx is not None and source_tail is not None:
        with PromptPrefixPatcher(
            model,
            patch_layer_idx,
            source_tail,
            prompt_len=prompt_len,
            patch_k=patch_k,
            patch_all_tokens=patch_all_tokens,
        ):
...
@torch.inference_mode()
def score_two_options_qwen_full_forward(
    model,
    processor,
    prompt: str,
    image: Image.Image,
    ...
    gold_lp = logprob_qwen_full_forward(
        model,
        processor,
        prompt,
        image,
    ...
    wrong_lp = logprob_qwen_full_forward(
        model,
        processor,
        prompt,
        image,
...
def merge_layer_trace_shards(args, metrics: List[str]) -> None:
    shard_paths = [Path(build_shard_file_path(args.out, shard_idx, args.num_shards)) for shard_idx in range(args.num_shards)]
    ...
    for shard_path in shard_paths:
        ...
        total_skipped += int(payload.get("skipped", 0))

    if first_payload is None:
        raise SystemExit("No shard payloads found to merge.")

    n_layers = 0
    if all_records:
        n_layers = len(all_records[0].get("layers", {}))
    elif first_payload.get("layer_scores"):
        n_layers = len(next(iter(first_payload["layer_scores"].values())))

    layer_deltas = {m: [0.0 for _ in range(n_layers)] for m in metrics}
    for rec in all_records:
        accumulate_trace_record(layer_deltas, rec, metrics)

    used = len(all_records)
    if used > 0:
        for m in metrics:
    ...
    merged_config = dict(first_payload.get("config", {}))
    merged_config["out"] = args.out
    if args.plot_prefix:
        merged_config["plot_prefix"] = args.plot_prefix
    if args.scan_plan_out:
        merged_config["scan_plan_out"] = args.scan_plan_out
    merged_config["num_shards"] = int(args.num_shards)
    merged_config["shard_idx"] = 0
    merged_config["merge_shards"] = True
    ...
    print("Merged shard outputs into:", args.out)
    print(json.dumps({"used": used, "skipped": total_skipped, "metrics": metrics, "num_shards": args.num_shards}, indent=2))


def resume_key_for_trace_row(idx: int, img_path: str, question: str, gold: str, wrong: str, position: str, trace_mode: str) -> str:
    return f"{idx}\t{img_path}\t{question}\t{gold}\t{wrong}\t{position}\t{trace_mode}"
...
def accumulate_trace_record(layer_deltas: Dict[str, List[float]], rec: Dict[str, Any], metrics: List[str]) -> None:
    ...
    base_effect = {
        m: metric_from_scores(m, rec["ctx"], gold, wrong, trace_mode)
        - metric_from_scores(m, rec["nc"], gold, wrong, trace_mode)
        for m in metrics
    }
    for layer_idx_str, patched in rec["layers"].items():
        ...
        for m in metrics:
            patched_effect = (
                metric_from_scores(m, patched, gold, wrong, trace_mode)
                - metric_from_scores(m, rec["nc"], gold, wrong, trace_mode)
            )
            layer_deltas[m][layer_idx] += float(abs(base_effect[m]) - abs(patched_effect))
    ...
    for i, layer in enumerate(layers):
        def make_hook(ii):
            def hook(module, inp, out):
                hs = out[0] if isinstance(out, (tuple, list)) else out
                if patch_all_tokens:
                    cache[ii] = hs.detach().cpu()
                else:
                    kk = min(patch_k, hs.size(1))
                    cache[ii] = hs[:, -kk:, :].detach().cpu()
            return hook
...
def main():
    ...
    if args.num_shards > 1:
        shard_scope = build_shard_token(args.shard_idx, args.num_shards)
        if not args.merge_shards:
            args.out = build_shard_file_path(args.out, args.shard_idx, args.num_shards)
            if args.plot_prefix:
                args.plot_prefix = build_shard_prefix_path(args.plot_prefix, args.shard_idx, args.num_shards)
            if args.scan_plan_out:
                args.scan_plan_out = build_shard_file_path(args.scan_plan_out, args.shard_idx, args.num_shards)
    ...
    if args.merge_shards:
        merge_layer_trace_shards(args, metrics)
        return
    ...
    qwen_full_forward_mode = is_qwen_full_forward_model(model)
    layers = _get_layers(model)
    n_layers = len(layers)
    ...
    shard_rows = list(iter_rows_for_shard(rows, args.num_shards, args.shard_idx))

    for row_idx, row in tqdm(shard_rows, total=len(shard_rows), desc=f"Tracing {shard_scope or 'full'}"):
        gold = norm_text(row["gold"])
        wrong = norm_text(row["wrong"])
        question = str(row["question"])
        row_key = resume_key_for_trace_row(
            row_idx,
            str(resolve_image_path(row, args.image_root) or row.get("image_path", "")),
            question,
            gold,
            wrong,
            args.position,
        ...
        try:
            ...
            # Keep per-row inputs on CPU; move them to GPU only inside the active forward pass.
            nc_inputs = build_inputs_mm(processor, nc_prompt, image, "cpu")
            ctx_inputs = build_inputs_mm(processor, ctx_prompt, image, "cpu")

            if qwen_full_forward_mode:
                nc_scores = score_two_options_qwen_full_forward(
                    model, processor, nc_prompt, image, gold, wrong
                )
                ctx_scores = score_two_options_qwen_full_forward(
            ...
            for layer_idx in range(n_layers):
                if qwen_full_forward_mode:
                    patched = score_two_options_qwen_full_forward(
                        model,
                        processor,
                        ctx_prompt,
                        image,
                        ...
                        source_tail=base_cache[layer_idx],
                        patch_k=args.patch_k,
                        patch_all_tokens=args.patch_all_tokens,
                    )
                else:
            ...
            if is_cuda_oom_error(exc):
                skipped += 1
                tracker.mark_done(row_key, {"status": "cuda_oom"})
                tracker.update(skipped=skipped, used=used)
                maybe_collect_cuda_garbage(used + skipped, every=1)
                continue
            raise
        finally:
            if patched_cache is not None:
            ...
            if ctx_cache is not None:
                del ctx_cache
            if nc_inputs is not None:
                del nc_inputs
            if ctx_inputs is not None:
                del ctx_inputs
            if image is not None:
                del image
            maybe_collect_cuda_garbage(used + skipped + 1, every=1)
