...
def main():
    ...
    for row in tqdm(rows, desc="Build samples"):
        ...
        # Keep per-sample inputs on CPU; build_prompt_cache_from_inputs moves one sample at a time
        # onto the model device. This avoids pinning the entire dataset's image tensors in VRAM.
        nc_inputs = build_inputs_mm(processor, nc_prompt, image, "cpu", vision_cache_key=vision_cache_key)
        ctx_inputs = build_inputs_mm(processor, ctx_prompt, image, "cpu", vision_cache_key=vision_cache_key)
