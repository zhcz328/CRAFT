...
def main():
    ...
    if tracker.is_done(str(out_path)) and out_path.exists():
        print(f"saved_plot={out_path}")
        tracker.finish(out_path=str(out_path))
        return
    plot_probe(args.summary_path, args.metric_key, args.title, args.out_path)
    tracker.mark_done(str(out_path), {"metric_key": args.metric_key})
    tracker.finish(out_path=str(out_path))
