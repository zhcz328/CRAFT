...
def main():
    ...
    ap.add_argument("--task", default="conflict", choices=["conflict", "follow_conflict"])
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[],
        help="Custom epoch checkpoints to save stage metrics, e.g. --stage_epochs 5 15 30",
    )
    ...
    if args.stage_epochs:
        stage_epochs = sorted(set(int(x) for x in args.stage_epochs if int(x) >= 1 and int(x) <= args.epochs))
        if not stage_epochs:
            raise ValueError("No valid --stage_epochs. Values must be in [1, --epochs].")
    else:
        stage_epochs = sorted(set([max(1, args.epochs // 3), max(1, (2 * args.epochs) // 3), args.epochs]))
    ...
    for layer_idx in layer_progress:
        ...
        for epoch_idx in epoch_progress:
            ...
            if epoch_idx in stage_epochs and epoch_idx not in stage_val_metrics:
                stage_val_metrics[epoch_idx] = copy.deepcopy(val_metrics)
        ...
        # If early stopping happened before some stage epochs, backfill with
        # the last observed validation metrics so all 3 stages are always present.
        for stage_epoch in stage_epochs:
            if stage_epoch not in stage_val_metrics and last_val_metrics is not None:
                stage_val_metrics[stage_epoch] = copy.deepcopy(last_val_metrics)
        ...
        layer_metrics = {
            ...
            "stage_val_metrics": {
                str(epoch): stage_val_metrics[epoch] for epoch in stage_epochs if epoch in stage_val_metrics
            },
        }
