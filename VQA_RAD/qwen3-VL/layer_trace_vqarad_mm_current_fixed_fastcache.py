...
def build_prompt_cache_from_inputs(model, base_inputs, need_hidden_states: bool = False):
    out = model(**base_inputs, use_cache=True, output_hidden_states=need_hidden_states)
...
def build_patched_prompt_cache_from_inputs(
    ...
    patch_k: int,
    patch_all_tokens: bool = False  # 新增参数
):
    """
    构建 patched prompt cache，可以选择 patch 尾部 k 个 token 或整层 token
    """
    # 这个 hook 内部会根据 patch_all_tokens 决定替换多少 token
    class PatchHook:
        def __init__(self):
            self.handle = None
        ...
        def hook_fn(self, module, inp, out):
            ...
            if patch_all_tokens:
                # patch 整层所有 token
                hs2[:, :, :] = source_tail.to(hs2.device, dtype=hs2.dtype)
            else:
                # patch 最后 patch_k 个 token
...
def plot_scores(layer_scores: Dict[str, List[float]], out_prefix: str):
    ...
    for metric, vals in layer_scores.items():
        ...
        plt.close()
def capture_layer_outputs_lastk(model, base_inputs, patch_k):
    layers = _get_layers(model)
    ...
    with torch.inference_mode():
        _ = model(**base_inputs, use_cache=False, output_hidden_states=False)
...
def main():
    ...
    "--patch_all_tokens",
    action="store_true",
    help="如果设置，patch 整层所有 token，而不仅仅是最后 k 个 token"
    )
    ...
    for row in tqdm(rows, desc="Tracing"):
        ...
        base_cache = capture_layer_outputs_lastk(model, nc_inputs, args.patch_k)
