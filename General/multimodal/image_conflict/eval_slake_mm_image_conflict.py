...
from mask_utils import UNKNOWN_ANSWER, build_masked_image, normalize_answer
...
def main():
    ...
    if not args.out_dir:
        dataset_slug = re.sub(r"[^a-zA-Z0-9._-]+", "_", str(args.dataset_name or "gqa"))
        args.out_dir = str(Path(_model_spec.slug) / dataset_slug / "eval-results_slake_image_conflict")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluate NC/IC"):
        ...
        if img_path is None or (not args.mask_all and not args.add_noise_all and (not target_labels or det_path is None)):
            skipped += 1
