    ...
    try:
        ...
        for sample_idx, s in enumerate(samples, start=1):
            ...
            if include_nc:
                ...
                nc_ab_scores = s["nc_scores"]
                ab_nc_margin = s["base_nc_gold_margin"]
                nc_damage = 0.0
...
def main():
    ...
    def run_round(scan_layers: List[int], out_path: str, round_name: str):
        ...
        coarse_scores = tracker.read_records(f"{round_name}_coarse.jsonl") if args.resume else []
        coarse_done_heads = {(int(item["layer"]), int(item["head"])) for item in coarse_scores}
        for layer_idx, head_idx in tqdm(all_heads, desc=f"Coarse {round_name}", unit="head"):
            coarse_key = resume_key_for_head(round_name, "coarse", layer_idx, head_idx)
            if tracker.is_done(coarse_key) or (int(layer_idx), int(head_idx)) in coarse_done_heads:
                continue
            ...
            coarse_item = {
                "layer": layer_idx,
                "head": head_idx,
                "coarse_mean_abs_effect_reduction": stats["mean_hallucination_relief"],
                "coarse_mean_abs_base_change": stats["mean_nc_gold_margin_damage"],
                "coarse_mean_ic_follow_context_gain": stats["mean_ic_follow_context_gain"],
            }
            coarse_scores.append(coarse_item)
            coarse_done_heads.add((int(layer_idx), int(head_idx)))
            tracker.append_record(f"{round_name}_coarse.jsonl", coarse_item)
            tracker.mark_done(
                coarse_key,
                {"round": round_name, "stage": "coarse", "layer": int(layer_idx), "head": int(head_idx)},
            )
            tracker.update(current_round=round_name, current_stage="coarse", coarse_completed=len(coarse_scores))
            maybe_empty_cuda_cache(len(coarse_scores), every=1)

        coarse_scores.sort(
            key=lambda r: (-(r["coarse_mean_abs_effect_reduction"]), r["layer"], r["head"])
        )

        coarse_rerank_scores = tracker.read_records(f"{round_name}_coarse_rerank.jsonl") if args.resume else []
        rerank_done_heads = {(int(item["layer"]), int(item["head"])) for item in coarse_rerank_scores}
        if args.coarse_topk and args.coarse_topk > 0:
            ...
            for item in tqdm(rerank_pool, desc=f"Rerank {round_name}", unit="head"):
                ...
                rerank_key = resume_key_for_head(round_name, "coarse_rerank", layer_idx, head_idx)
                if tracker.is_done(rerank_key) or (layer_idx, head_idx) in rerank_done_heads:
                    continue
                stats = evaluate_single_head(
                    model=model,
                    processor=processor,
                ...
                coarse_rerank_scores.append(rerank_item)
                rerank_done_heads.add((layer_idx, head_idx))
                tracker.append_record(f"{round_name}_coarse_rerank.jsonl", rerank_item)
                tracker.mark_done(
                    rerank_key,
                    {"round": round_name, "stage": "coarse_rerank", "layer": int(layer_idx), "head": int(head_idx)},
                )
                tracker.update(current_round=round_name, current_stage="coarse_rerank", rerank_completed=len(coarse_rerank_scores))
                maybe_empty_cuda_cache(len(coarse_rerank_scores), every=1)
        ...
        refine_done_heads = {(int(item["layer"]), int(item["head"])) for item in results}
        for layer_idx, head_idx in tqdm(candidate_heads, desc=f"Refine {round_name}", unit="head"):
            refine_key = resume_key_for_head(round_name, "refine", layer_idx, head_idx)
            if tracker.is_done(refine_key) or (int(layer_idx), int(head_idx)) in refine_done_heads:
                continue
            stats = evaluate_single_head(
                model=model,
                processor=processor,
            ...
            results.append(item)
            refine_done_heads.add((int(layer_idx), int(head_idx)))
            tracker.append_record(f"{round_name}_results.jsonl", item)
            tracker.mark_done(
                refine_key,
                {"round": round_name, "stage": "refine", "layer": int(layer_idx), "head": int(head_idx)},
            )
            ...
            tracker.update(current_round=round_name, current_stage="refine", refined_completed=len(results))
