...
def default_preds_path(project_root, position, model_name="", model="hulumed-4b", dataset_name="gqa"):
    from model_registry import resolve_model_selection, sanitize_path_component

    _, spec = resolve_model_selection(model_name=model_name, model=model)
    dataset_slug = sanitize_path_component(dataset_name or "gqa")
    root = Path(project_root) / spec.slug / dataset_slug / "eval-results_slake_image_conflict"
