...
def pareto_front(rows):
    ...
    for i, ri in enumerate(rows):
        ei = ri["mean_abs_effect_reduction"]
        bi = ri["mean_abs_base_change"]
        dominated = False
        for j, rj in enumerate(rows):
            if i == j:
                continue
            ej = rj["mean_abs_effect_reduction"]
            bj = rj["mean_abs_base_change"]
            if (ej >= ei and bj <= bi) and (ej > ei or bj < bi):
                dominated = True
                break
        if not dominated:
            front.append(ri)
...
def sort_selected(rows, mode):
    if mode == "backbone":
        ...
        rows.sort(key=lambda x: (-x["mean_abs_effect_reduction"], x["mean_abs_base_change"], -x["conflict_specific_score"], x["layer"], x["head"]))
    else:
...
def main():
    ...
    ap.add_argument("--mode", default="conflict_specific",
                    choices=["conflict_specific", "backbone", "custom", "pareto"])
    ...
    if tracker.is_done("select") and out_json_path.exists() and out_csv_path.exists():
        print(f"[OK] selected (resume) -> {args.out_json}, {args.out_csv}")
        tracker.finish(out_json=args.out_json, out_csv=args.out_csv)
        return
    ...
    tracker.mark_done("select", {"n_selected": len(selected), "n_candidates": len(rows)})
    tracker.finish(out_json=args.out_json, out_csv=args.out_csv, n_selected=len(selected), n_candidates=len(rows))
