...
class InternVLForwardAdapter(torch.nn.Module):
    ...
    def _infer_vision_device(self) -> torch.device | None:
        try:
            core = self.base_model.get_model() if hasattr(self.base_model, "get_model") else self.base_model
    ...
    def forward(
        ...
        **kwargs,
        ...
        vision_device = self._infer_vision_device()
        if vision_device is not None and pixel_values.device != vision_device:
        ...
        elif image_flags.device != pixel_values.device:
            image_flags = image_flags.to(device=pixel_values.device)
