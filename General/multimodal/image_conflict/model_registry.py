...
def default_eval_results_dir(
    ...
    return Path(project_root) / spec.slug / dataset_slug / "eval-results_slake_all"
...
def prefix_model_relative_path(path_value: str, model_name: str = "", model: str = "") -> str:
    ...
    if path.parts and path.parts[0] == spec.slug:
        return str(path)
    if len(path.parts) >= 2 and path.parts[0] == "data" and path.parts[1] == spec.slug:
        return str(path)
