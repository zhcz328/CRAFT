...
from mask_utils import UNKNOWN_ANSWER, build_masked_image, is_unknown_text
...
def load_row_image(row, project_root, image_size, mask_scale=1.0):
    ...
    detection_path = resolve_image_path(project_root, row["detection_path"]) if str(row.get("detection_path", "")).strip() else None
    mask_path = resolve_image_path(project_root, row["mask_path"]) if str(row.get("mask_path", "")).strip() else None
