...
def normalize_hallucination_label(label):
    text = str(label or "").strip().lower()
    if text in {"hallucination", "follow_conflict", "resist", "non-unknown", "non_unknown"}:
...
def build_rows(split_rows, split_name, position, outcome_map, tracker=None):
    ...
    for record in existing_records:
        ...
        if record.get("matched"):
            matched += 1
    progress = tqdm(split_rows, total=len(split_rows), desc=f"build {split_name} manifests", unit="sample")
    for row in progress:
        img_id = row.get("img_id", "")
        question = row.get("question", "")
        gold = row.get("gold_norm") or row.get("gold") or row.get("answer") or ""
        wrong = row.get("unknown_target") or UNKNOWN_ANSWER
        nc_candidates = parse_json_list(row.get("nc_candidates"))
        wrong_candidates = []
        seen_candidates = set()
        for candidate in nc_candidates:
            candidate_norm = str(candidate or "").strip().lower()
            if not candidate_norm or candidate_norm in seen_candidates:
                continue
            if candidate_norm == str(wrong).strip().lower():
                continue
            seen_candidates.add(candidate_norm)
            wrong_candidates.append(candidate_norm)
        competing_answers = list(wrong_candidates)
        gold_norm = str(gold).strip().lower()
        if gold_norm and gold_norm not in seen_candidates:
            competing_answers.append(gold_norm)
        ...
        if outcome_rec:
            matched += 1
        follow_label = normalize_hallucination_label(derive_follow_label(outcome_rec, gold, wrong))
        ...
        shared = {
            ...
            "follow_target": HALLUCINATION_TARGETS[follow_label],
            "hallucination_label": follow_label,
            "hallucination_target": HALLUCINATION_TARGETS[follow_label],
        ...
        prompts = {
            "nc": {"image_variant": "nc", "prompt_text": prompt_text},
            "ic": {"image_variant": "ic", "prompt_text": prompt_text},
        }
        sample_rows = []
        for prompt_type, prompt_spec in prompts.items():
            sample_rows.append(
                {
                    **shared,
                    "record_id": f"{shared['sample_id']}:{prompt_type}",
                    "prompt_type": prompt_type,
                    "image_variant": prompt_spec["image_variant"],
...
def summarize(rows, matched_samples, total_samples):
    summary = {
        "n_records": len(rows),
        "prompt_type_counts": {"nc": 0, "ic": 0},
        "matched_outcome_samples": matched_samples,
        "total_samples": total_samples,
        "hallucination_counts_ic_only": {"hallucination": 0, "unknown": 0},
    }
    for row in rows:
        summary["prompt_type_counts"][row["prompt_type"]] += 1
        if row["prompt_type"] == "ic":
            label = normalize_hallucination_label(row.get("hallucination_label", row.get("follow_label", "unknown")))
            summary["hallucination_counts_ic_only"][label] += 1
    return summary


def main():
    ap = argparse.ArgumentParser(description="Prepare image-conflict tuned-lens manifests.")
