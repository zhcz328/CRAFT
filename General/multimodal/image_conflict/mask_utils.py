...
_DETECTION_JSON_CACHE: dict[str, Any] = {}
...
def read_detection_map(path: str | Path, image_id: str | None = None) -> Dict[str, List[float]]:
    det_path = Path(path)
    if not det_path.exists():
        return {}
    cache_key = str(det_path.resolve())
    raw = _DETECTION_JSON_CACHE.get(cache_key)
    if raw is None:
        raw = json.loads(det_path.read_text(encoding="utf-8"))
        _DETECTION_JSON_CACHE[cache_key] = raw
    ...
    # Flat detection map: {label_or_id: [x,y,w,h]}
    flat_like = True
    ...
    # GQA scene graph style: {image_id: {width,height,objects:{obj_id:{x,y,w,h,name,...}}}}
    if image_id is None:
        return out
    graph = raw.get(str(image_id))
...
def build_masked_image(
    ...
    if mask_all:
        ...
        detection_map = read_detection_map(detection_path, image_id=image_id)
