...
FLIP_MODE="${FLIP_MODE:-non_unknown_to_unknown}"
...
TL_ROOT="${TL_ROOT:-/root/autodl-tmp/image_conflict/tuned_lens/${MODEL_SLUG}}"
...
if [[ -n "${PROBE_TASKS:-}" ]]; then
  ...
  RAW_PROBE_TASKS="hallucination conflict"
fi

PROBE_TASK_LIST=()
for probe_task in $RAW_PROBE_TASKS; do
  case "$probe_task" in
    hallucination|follow_conflict|conflict)
      PROBE_TASK_LIST+=("$probe_task")
      ;;
    *)
      echo "Unsupported PROBE_TASK/PROBE_TASKS item: $probe_task" >&2
      echo "Use one or more of: hallucination, conflict" >&2
      exit 1
      ;;
  esac
done
