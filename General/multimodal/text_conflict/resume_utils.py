...
_SHARD_SCOPE_RE = re.compile(r"^shard\d+of\d+$")


def _is_shard_scope_dir(path: str | Path) -> bool:
    return bool(_SHARD_SCOPE_RE.match(Path(path).name))


def list_compatible_resume_dirs(resume_dir: str | Path) -> list[Path]:
    ...
    if _is_shard_scope_dir(resume_dir):
        parent = resume_dir.parent
        add_dir(parent)
        if parent.exists():
            for child in sorted(parent.iterdir()):
                if child.is_dir() and _is_shard_scope_dir(child):
                    add_dir(child)
    elif resume_dir.exists():
        for child in sorted(resume_dir.iterdir()):
            if child.is_dir() and _is_shard_scope_dir(child):
                add_dir(child)
...
class ResumeTracker:
    ...
    def __post_init__(self) -> None:
        ...
        self.read_dirs = list_compatible_resume_dirs(self.resume_dir)
        ...
        if self.enabled:
            ...
            for read_dir in self.read_dirs:
                for item in read_jsonl(read_dir / "completed_keys.jsonl"):
