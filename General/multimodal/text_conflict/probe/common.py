...
def default_preds_path(project_root, position, model_name="", model="hulumed-4b", dataset_name="gqa"):
    results_root = default_eval_results_dir(project_root, model_name=model_name, model=model, dataset_name=dataset_name)

    legacy_flat = results_root / position / "preds.jsonl"
    if legacy_flat.exists():
        return legacy_flat

    if results_root.exists():
        direct_matches = sorted(
            results_root.glob(f"*/{position}/preds.jsonl"),
    ...
    new_layout = results_root.parent / f"result_{position}_slake" / "eval-results_slake_all" / "preds.jsonl"
