# -*- coding: utf-8 -*-
"""
SLAKE multimodal SUPPORT vs CONFLICT evaluation (NO 'unknown') - FINAL (image tokens aligned)

- Model: Qwen3-VL-8B-Instruct (local)
- Data: slake_*_closed.csv (CLOSED; may include non-yes/no answers)
- For each sample:
...
def score_two_options_mm(model, processor, messages, opt_a: str, opt_b: str):
    if is_qwen35_model(model):
        return {
            opt_a: logprob_qwen35_full_forward(model, processor, messages, " " + str(opt_a)),
            opt_b: logprob_qwen35_full_forward(model, processor, messages, " " + str(opt_b)),
        }
...
def resume_key_for_eval_row(idx: int, img_id: str, question: str, gold: str, conflict_ans: str, position: str) -> str:
    return f"{idx}\t{img_id}\t{question}\t{gold}\t{conflict_ans}\t{position}"
...
def main():
    ...
    args = ap.parse_args()
    args.model, model_spec = resolve_model_selection(model_name=args.model_name, model=args.model)
    if not args.out_dir:
        dataset_slug = sanitize(args.dataset_name or "gqa")
        args.out_dir = str(Path(model_spec.slug) / dataset_slug / "eval-results_slake_all")
    ...
    if "answer_type" in df.columns:
        ...
        df = df[df["answer_type_norm"] == "YES/NO"].copy()
    ...
    N = int(tracker.state.get("N", 0))
    skipped_missing_img = int(tracker.state.get("skipped_missing_img", 0))

    support_correct = int(tracker.state.get("support_correct", 0))
    conflict_still_gold = int(tracker.state.get("conflict_still_gold", 0))
    conflict_follow_evidence = int(tracker.state.get("conflict_follow_evidence", 0))
    resist_cnt = int(tracker.state.get("resist_cnt", 0))
    changed_cnt = int(tracker.state.get("changed_cnt", 0))

    resist_given_sup_correct = int(tracker.state.get("resist_given_sup_correct", 0))
    changed_given_sup_correct = int(tracker.state.get("changed_given_sup_correct", 0))
    follow_conflict_given_sup_correct = int(tracker.state.get("follow_conflict_given_sup_correct", 0))

    sup_margin_sum = float(tracker.state.get("sup_margin_sum", 0.0))
    con_margin_sum = float(tracker.state.get("con_margin_sum", 0.0))
    nc_correct = int(tracker.state.get("nc_correct", 0))
    nc_margin_sum = float(tracker.state.get("nc_margin_sum", 0.0))
    
    # conditional on NC being correct
    resist_given_nc_correct = int(tracker.state.get("resist_given_nc_correct", 0))
    changed_given_nc_correct = int(tracker.state.get("changed_given_nc_correct", 0))
    follow_conflict_given_nc_correct = int(tracker.state.get("follow_conflict_given_nc_correct", 0))
    nc_correct_cnt = int(tracker.state.get("nc_correct_cnt", 0))
    both_correct_rows = tracker.read_records("both_correct_rows.jsonl")
    unknown_conflict_cnt = int(tracker.state.get("unknown_conflict_cnt", 0))
    # --- NC baseline stats (DO NOT use support as reference) ---
    resist_vs_nc_cnt = int(tracker.state.get("resist_vs_nc_cnt", 0))
    changed_vs_nc_cnt = int(tracker.state.get("changed_vs_nc_cnt", 0))

    conflict_pred_unknown_cnt = int(tracker.state.get("conflict_pred_unknown_cnt", 0))          # con_pred == unknown
    changed_to_unknown_vs_nc_cnt = int(tracker.state.get("changed_to_unknown_vs_nc_cnt", 0))       # nc_pred != unknown but con_pred == unknown
    preds_mode = "a" if args.resume else "w"
    with open(preds_path, preds_mode, encoding="utf-8") as wf:
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="MM SUPPORT/CONFLICT eval (final)"):
            ...
            row_key = resume_key_for_eval_row(idx, img_id, question, gold, conflict_ans, args.position)
            ...
            if "image_path" in row and pd.notna(row["image_path"]):
                candidates.extend(build_path_candidates(row["image_path"], args.image_root))
            if "img_name" in row and pd.notna(row["img_name"]):
                candidates.extend(build_path_candidates(row["img_name"], args.image_root))
            candidates.extend(build_path_candidates(img_id, args.image_root))
            ...
            if img_path is None:
                ...
                tracker.update(
                    ...
                    conflict_still_gold=conflict_still_gold,
                    conflict_follow_evidence=conflict_follow_evidence,
                    resist_cnt=resist_cnt,
                    changed_cnt=changed_cnt,
                    resist_given_sup_correct=resist_given_sup_correct,
                    changed_given_sup_correct=changed_given_sup_correct,
                    follow_conflict_given_sup_correct=follow_conflict_given_sup_correct,
                    sup_margin_sum=sup_margin_sum,
                    con_margin_sum=con_margin_sum,
                    nc_correct=nc_correct,
                    nc_margin_sum=nc_margin_sum,
                    resist_given_nc_correct=resist_given_nc_correct,
                    changed_given_nc_correct=changed_given_nc_correct,
                    follow_conflict_given_nc_correct=follow_conflict_given_nc_correct,
                    nc_correct_cnt=nc_correct_cnt,
                    unknown_conflict_cnt=unknown_conflict_cnt,
                    resist_vs_nc_cnt=resist_vs_nc_cnt,
                    changed_vs_nc_cnt=changed_vs_nc_cnt,
                    conflict_pred_unknown_cnt=conflict_pred_unknown_cnt,
                    changed_to_unknown_vs_nc_cnt=changed_to_unknown_vs_nc_cnt,
                    both_correct_cnt=len(both_correct_rows),
                )
                continue
            ...
            if (nc_pred == gold) and (sup_pred == gold):
                ...
                both_correct_rows.append(both_correct_row)
                tracker.append_record("both_correct_rows.jsonl", both_correct_row)
            if nc_pred == gold:
                nc_correct += 1
                nc_correct_cnt += 1
            ...
            wf.write(json.dumps(pred_record, ensure_ascii=False) + "\n")
