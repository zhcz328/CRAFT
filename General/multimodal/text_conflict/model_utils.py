...
def iter_model_loader_classes(model_name: str = ""):
    ...
    elif spec is not None and spec.family == "qwen3_vl":
        names = [
...
def _model_has_logits_head(model) -> bool:
    candidates = [model]
    ...
    for cand in candidates:
        if hasattr(cand, "lm_head"):
            return True
        get_out = getattr(cand, "get_output_embeddings", None)
        if callable(get_out):
            try:
    ...
    for cls in iter_model_loader_classes(model_name):
        try:
            ...
            try:
                ...
                if torch_dtype is None or "dtype" not in str(exc):
                    raise
            ...
            if not _model_has_logits_head(model):
                raise RuntimeError(
                    f"{cls.__name__} loaded a backbone model without LM head "
                    f"for {model_name}. Try another loader."
                )
            return model
