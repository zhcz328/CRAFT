...
def _make_head_mask_pre_hook(layer_idx: int, heads_to_mask: List[int], num_heads: int, keep_mode: str = "self"):
    ...
    def _hook(module, args, kwargs):
        ...
        # 鏈熸湜鏀寔 [bs, 1, q, k] 鎴?[bs, num_heads, q, k]
        if attn_mask.dim() != 4:
            return
...
def score_continuation_with_cache(
    ...
    """
    continuation 鐨勭涓€涓?token 鐢?prompt_last_logits 璁″垎锛?    鍚庣画 token 鐢?past_key_values 鎺ョ潃绠椼€?    杩斿洖鏁翠釜 continuation 鐨?log-prob sum銆?    """
    ...
    rest_inp = cont_ids[:, :-1]  # 璁╂ā鍨嬮娴嬬2..T涓猼oken
    out = model(
        input_ids=rest_inp,
    ...
    try:
        for s in tqdm(samples, desc="Ablate selected heads"):
            ...
            if tracker.is_done(sample_key):
                continue
            # mask_scope: all => nc/ctx 閮介噸绠?            # ctx_only => 鍙噸绠?ctx锛宯c 鐢?baseline
