...
OUT_DIR = BUNDLE_ROOT / "outputs" / "_fig4"
...
def draw(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    plt.rcParams.update(
        {
            ...
            "xtick.labelsize": 11 * FONT_SCALE * NUMERIC_FONT_SCALE,
            "ytick.labelsize": 11 * FONT_SCALE * NUMERIC_FONT_SCALE,
            "axes.linewidth": 1.1,
        }
    )

    fig, axes = plt.subplots(1, 4, figsize=(19.8, 5.9), sharey=True)
    fig.subplots_adjust(left=0.055, right=0.985, bottom=0.24, top=0.88, wspace=0.30)
    ...
    for ax, panel in zip(axes, PANELS):
        ...
        ax.tick_params(length=3.5, width=0.9, labelleft=True, labelbottom=True)
        ax.set_ylabel("")
        ...
        if onsets:
            ...
            _ = left_frac, middle_frac, right_frac, phase_y, phase_text_style
            _ = PANEL_LENC_TEXT_X.get(panel["title"], l_enc - 0.55), l_star, primary_range
    ...
    # Main figure legend intentionally removed; legend is exported separately.
    ...
    legend_png_path = OUT_DIR / "fig4_layer_trace_legend.png"
    legend_pdf_path = OUT_DIR / "fig4_layer_trace_legend.pdf"
    fig_leg = plt.figure(figsize=(8.0, 1.2))
    ...
    fig_leg.savefig(legend_png_path, dpi=300, bbox_inches="tight")
    fig_leg.savefig(legend_pdf_path, bbox_inches="tight")
    plt.close(fig_leg)
    plt.close(fig)
    ...
    manifest["outputs"] = {
        ...
        "legend_pdf": str(legend_pdf_path),
        "sources": str(meta_path),
    }
