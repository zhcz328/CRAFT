...
def draw(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for ax, spec in zip(axes_flat, PANELS):
        ...
        if x_thr is not None:
            ...
            ax.text(x_thr + 0.015 * x_limit, 0.055 * y_limit, r"$\tau_{\mathrm{bcp}}$", fontsize=9, ha="left", va="bottom")
        if y_thr is not None:
            ax.axhline(y_thr, color="black", linestyle="--", linewidth=1.0)
            ax.text(0.98 * x_limit, y_thr + 0.025 * y_limit, r"$\tau_{\mathrm{cer}}$", fontsize=9, ha="right", va="bottom")

        ax.set_title(spec["title"], pad=4)
        if x_thr is not None and y_thr is not None:
            left_x = 0.30 * x_thr
            right_x = x_thr + 0.48 * (x_limit - x_thr)
            top_y = y_thr + 0.64 * (y_limit - y_thr)
            bottom_y = 0.42 * y_thr
            ax.text(left_x, top_y, "Causal\nKey Heads", ha="left", va="center", fontsize=8.5, linespacing=0.9)
            ax.text(right_x, top_y, "Backbone\nHeads", ha="center", va="center", fontsize=8.5, linespacing=0.9)
            ax.text(right_x, bottom_y, "Backbone\nHeads", ha="center", va="center", fontsize=8.5, linespacing=0.9)
            ax.text(left_x, bottom_y, "Irrelevant\nHeads", ha="left", va="center", fontsize=8.5, linespacing=0.9)
    ...
    axes_flat[0].set_ylabel("CER")
    for ax in axes_flat:
        ax.set_xlabel("BCP")
    fig.subplots_adjust(left=0.055, right=0.995, bottom=0.20, top=0.86, wspace=0.26)
