...
DEFAULT_MODEL = "CultriX/Qwen3-8B-Hippocratesv1"  # HF id
...
def pick_yes_no_by_logit(model, tokenizer, prompt_text: str, enable_thinking: bool):
    ...
    """
    # Try common variants to reduce tokenization/prior bias
    yes_lp = max(
...
def main():
    ...
    ap.add_argument("--model", default=DEFAULT_MODEL, help=f"HF id or local path (default: {DEFAULT_MODEL})")
    ap.add_argument("--trust_remote_code", action="store_true",
                    help="Allow custom model code from the Hub (enable if you trust the repo).")
    ap.add_argument("--dtype", choices=["auto", "float16", "bfloat16", "float32"], default="auto",
                    help="Model dtype. auto prefers bf16 if supported, else fp16 on CUDA.")
    ...
    ap.add_argument("--local_file", default="",
                    help="Path to local DriftMed jsonl (ConflictMed.jsonl / ConflictMed_v2.jsonl). If set, ignore --config remote loading.")
    ...
    # Pair metrics (rule: wrong_id = correct_id + 2145)
