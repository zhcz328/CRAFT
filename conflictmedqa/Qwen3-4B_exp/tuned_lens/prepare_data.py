...
def build_rows(pairs, split_name, position, outcome_map):
    ...
    for pair in pairs:
        ...
        for side in ("correct", "wrong"):
            ...
            prompts = {
                "base": base_prompt,
                "support": inject_evidence(base_prompt, make_evidence(gold == YES), position),
                "conflict": inject_evidence(base_prompt, make_evidence(wrong == YES), position),
            }
            for prompt_type, prompt_text in prompts.items():
                rows.append(
                    {
                        **shared,
                        "record_id": f"{pair_id}:{side}:{prompt_type}",
...
def main():
    ap = argparse.ArgumentParser(description="Prepare prompt manifests for tuned lens training.")
