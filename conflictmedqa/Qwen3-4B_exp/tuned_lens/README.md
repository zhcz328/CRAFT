...
python tuned_lens/train_tuned_lens.py \
  ...
  --out_dir /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question

python tuned_lens/analyze_trajectories.py \
  --manifest tuned_lens/data/val_pair_stratified_before_question.jsonl \
  --model /path/to/Qwen3-4B \
  --lens_ckpt /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question/tuned_lens.pt \
  --prompt_types conflict \
  --out_dir /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question/trajectory

python tuned_lens/plot_alignment_figure.py \
  --manifest tuned_lens/data/val_before_question.jsonl \
  --model /path/to/Qwen3-4B \
  --comparison_target final_hidden \
  --alignment_mode tuned_lens \
  --lens_ckpt /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question/tuned_lens.pt \
  --layer 20 \
  --prompt_types conflict \
  --out_dir /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question/alignment_hidden
...
python tuned_lens/plot_alignment_figure.py \
  ...
  --layer 20 \
  --prompt_types conflict \
  --out_dir /root/autodl-tmp/tuned_lens/conflictmedqa/qwen3-4b/before_question/alignment
```
...
- If you want a paper-like vector-space figure, prefer `--comparison_target final_hidden`
