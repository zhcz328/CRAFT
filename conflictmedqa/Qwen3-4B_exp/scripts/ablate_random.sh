...
POSITIONS="${POSITIONS:-${POSITION:-before_answer}}"
...
ABLATE_SET="${ABLATE_SET:-random}"
RANDOM_SEED="${RANDOM_SEED:-42}"
RANDOM_MATCH="${RANDOM_MATCH:-total}"
...
if [[ -z "$HEAD_FILE" ]]; then
  if [[ -f "${MODEL_RESULT_DIR}/headscan_rounds_top30_inf/selected_heads.json" ]]; then
    HEAD_FILE="${MODEL_RESULT_DIR}/headscan_rounds_top30_inf/selected_heads.json"
  elif [[ -f "${MODEL_RESULT_DIR}/headscan_rounds_top30_inf/head_groups.json" ]]; then
    HEAD_FILE="${MODEL_RESULT_DIR}/headscan_rounds_top30_inf/head_groups.json"
  fi
fi
...
run_one_split() {
  ...
  CMD=(
    "$PYTHON_BIN" ablate_head_inf_random.py
    --pairs "$pairs_path"
    --model "$MODEL_PATH"
...
run_default_splits_for_position() {
  ...
  for split in "${SPLITS_ARR[@]}"; do
    ...
    out_path="${result_dir}/ablate_random_${position}_${split}.jsonl"
...
if [[ -n "$PAIRS_PATH" ]]; then
  ...
  if [[ -n "$ABLATE_OUT" ]]; then
    ...
    OUT_PATH="${MODEL_RESULT_DIR}/ablate_random_${POSITIONS//,/__}.jsonl"
  fi
...
for split in "${SPLITS_ARR[@]}"; do
  ...
  out_path="${MODEL_RESULT_DIR}/ablate_random_${POSITIONS//,/__}_${split}.jsonl"
