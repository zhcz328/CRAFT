...
def infer_default_head_file(model_name: str, positions: list[str]) -> Path:
    ...
    is_llama32 = any(token in model_l for token in ("llama32_3b", "llama-3.2", "llama3.2"))
    base_dir = script_dir / "llama32_3b" if is_llama32 else script_dir
    candidate_roots = [
...
def _flatten_layer2heads(layer2heads):
    """Return a set of (layer, head) tuples."""
    return {(int(l), int(h)) for l, hs in layer2heads.items() for h in hs}

def _layer2heads_to_list(layer2heads):
    return [{"layer": int(l), "heads": [int(h) for h in hs]} for l, hs in sorted(layer2heads.items())]

def sample_random_heads(
    target_layer2heads: dict,
    n_layers: int,
    n_heads: int,
    seed: int = 0,
    match: str = "per_layer",
    ...
    """
    Sample a random head set whose *size matches* the target set.

    match:
      - per_layer: for each layer, sample k = len(target[layer]) heads from that layer
      - total: sample k = total target heads across all layers

    By default we try to avoid overlapping with the target heads (unless pool is insufficient).
    """
    rng = random.Random(seed)
    target_pairs = _flatten_layer2heads(target_layer2heads)

    used = {}
    notes = {
        "seed": seed,
        "match": match,
        "allow_overlap_requested": bool(allow_overlap),
        "overlap_avoided": True,
        "fallback_overlap": False,
        "insufficient_pool_layers": [],
    }

    def candidates_for_layer(layer: int, avoid: bool):
        cand = list(range(n_heads))
        if avoid:
            banned = {h for (l, h) in target_pairs if l == layer}
            cand = [h for h in cand if h not in banned]
        return cand

    if match == "per_layer":
        for layer in range(n_layers):
            k = len(target_layer2heads.get(layer, []))
            if k <= 0:
                continue

            # Prefer to avoid overlap; if impossible, fall back to allowing overlap for that layer.
            avoid = (not allow_overlap)
            cand = candidates_for_layer(layer, avoid=avoid)
            if len(cand) < k:
                notes["insufficient_pool_layers"].append({"layer": layer, "need": k, "available": len(cand)})
                if not allow_overlap:
                    # fallback: allow overlap
                    notes["fallback_overlap"] = True
                    notes["overlap_avoided"] = False
                    cand = candidates_for_layer(layer, avoid=False)

            if len(cand) >= k:
                picked = rng.sample(cand, k)
            else:
                # Still not enough unique heads (e.g., k > n_heads). Best-effort: sample without replacement.
                picked = cand[:]

            used[layer] = sorted(set(picked))

    elif match == "total":
        total_k = sum(len(v) for v in target_layer2heads.values())
        all_pairs = [(l, h) for l in range(n_layers) for h in range(n_heads)]

        if not allow_overlap:
            pool = [p for p in all_pairs if p not in target_pairs]
            if len(pool) < total_k:
                # fallback: allow overlap
                notes["fallback_overlap"] = True
                notes["overlap_avoided"] = False
        ...
        if len(pool) >= total_k:
            picked_pairs = rng.sample(pool, total_k)
        else:
            picked_pairs = pool[:]

        for (l, h) in picked_pairs:
            used.setdefault(l, []).append(h)

        for l in list(used.keys()):
            used[l] = sorted(set(used[l]))
    else:
        raise ValueError(f"Unknown random match mode: {match}")
...
def main():
    ...
    ap.add_argument("--ablate_set", default="target", choices=["target", "random"],
                    help="Which head set to ablate: 'target' (conflict_specific) or 'random' (same count as target).")
    ap.add_argument("--random_seed", type=int, default=0, help="Random seed used when --ablate_set random")
    ap.add_argument("--random_match", default="per_layer", choices=["per_layer", "total"],
                    help="How to match the random head count: per_layer (recommended) or total")
    ap.add_argument("--allow_random_overlap", action="store_true",
                    help="Allow random head set to overlap with target heads (default tries to avoid overlap)")
    ...
    # Target heads (from selected_heads.json or legacy head_groups.json)
    target_layer2heads, groups_obj = load_ablation_heads(str(head_file_path))

    # Decide which head set to actually ablate
    random_notes = None
    if args.ablate_set == "random":
        n_heads_cfg, _ = _get_num_heads_and_hidden(model)
        _, n_layers = _find_self_attn_modules(model)
        ablate_layer2heads, random_notes = sample_random_heads(
            target_layer2heads,
            n_layers=n_layers,
            n_heads=n_heads_cfg,
            seed=args.random_seed,
            match=args.random_match,
            allow_overlap=args.allow_random_overlap,
        )
    else:
        ablate_layer2heads = target_layer2heads

    # install hooks once (fast)
    hook_handles = install_head_mask_hooks(model, ablate_layer2heads)
    ...
    meta = {
        ...
        "masked_conflict_specific": _layer2heads_to_list(target_layer2heads),
        "ablate_set": args.ablate_set,
        "masked_used": _layer2heads_to_list(ablate_layer2heads),
