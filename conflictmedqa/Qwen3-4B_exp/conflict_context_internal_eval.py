...
def logprob_continuation(model, tokenizer, prompt_text: str, continuation: str) -> float:
    """
    Keep scoring logic aligned with conflictmedqa_eval_local.py:
    log P(continuation | prompt_text) via teacher forcing.
    """
...
def score_yes_no_fast(model, tokenizer, prompt_text):
    # Match conflictmedqa_eval_local.py exactly: evaluate both spaced/non-spaced variants.
    yes_lp = max(
