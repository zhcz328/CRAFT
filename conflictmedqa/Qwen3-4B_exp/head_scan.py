...
def oproj_head_ablate_hook(head_idx: int, head_dim: int):
    """forward_pre_hook on o_proj: zero slice for one head in its input."""
...
def run_head_scan_round(model, tok, layers_mod, n_heads, head_dim,
    ...
    for layer_idx in scan_layers:
        ...
        for h in tqdm(range(n_heads), desc=f"Scan L{layer_idx}", unit="head"):
            handle = oproj.register_forward_pre_hook(oproj_head_ablate_hook(h, head_dim))
...
def summarize_head_groups(round_outputs, out_path: str, topk: int = 10,
    ...
    if fixed_mode:
        ...
        conflict = [
            e for e in entries
            if e["mean_abs_effect_reduction"] > cs_eff_min and e["mean_abs_base_delta_change"] < cs_base_max
        ]
...
def main():
    ...
    ap.add_argument("--groups_topk", type=int, default=30, help="how many heads to keep per group")
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.model, trust_remote_code=True, device_map="auto", torch_dtype=torch_dtype
    )
    model.eval()
