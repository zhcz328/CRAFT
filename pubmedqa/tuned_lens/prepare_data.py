...
def build_rows(records, split_name, position, outcome_map, q_field, c_field, y_field, id_field):
    ...
    for idx, record in enumerate(records):
        ...
        shared = {
            ...
            "position": position,
            "gold_answer": gold,
            "wrong_answer": opposite_label(gold),
        ...
        prompts = {
            "base": make_nc_prompt(str(question), allow_unknown=False),
            "support": make_support_prompt(str(question), context, gold, position, allow_unknown=False),
            "conflict": make_conflict_prompt(str(question), context, gold, position, allow_unknown=False),
        }
        for prompt_type, prompt_text in prompts.items():
            rows.append(
                {
                    **shared,
                    "record_id": f"{pair_id}:{prompt_type}",
...
def main():
    ap = argparse.ArgumentParser(description="Prepare prompt manifests for PubMedQA tuned lens training.")
