...
OUT_DIR="${OUT_DIR:-$(join_output_path "result_yesno/${POSITION}/ablate")}"
LIMIT="${LIMIT:-0}"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" ablate_selected_heads_yesno.py \
  ...
  --mask_scope all \
  --limit "$LIMIT" \
  --out_jsonl "${OUT_DIR}/ablate_selected_heads_train.jsonl" \
  --out_summary "${OUT_DIR}/ablate_selected_heads_train_summary.json"
