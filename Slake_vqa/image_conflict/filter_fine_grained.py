...
from mask_utils import UNKNOWN_ANSWER, extract_inline_mask_data, normalize_answer
...
def discover_wrong_csv(explicit_path: str, in_csv: str) -> Path | None:
...
def attach_wrong_answers(df: pd.DataFrame, wrong_csv: str, in_csv: str) -> tuple[pd.DataFrame, str, int]:
    out_df = df.copy()
    ...
    wrong_path = discover_wrong_csv(wrong_csv, in_csv)
    if wrong_path is None:
        raise FileNotFoundError(
            "Could not locate a wrong-answer backup CSV. "
            "Provide --wrong_csv or place one at image_conflict/data/slake_closed_all_wrong_backup.csv."
        )
...
def should_drop_unknown_for_closed(dataset: str, model) -> bool:
    return normalize_dataset_name(dataset) == "heal-medvqa" and is_internvl_model(model)
...
def maybe_attach_wrong_answers(df: pd.DataFrame, wrong_csv: str, in_csv: str) -> tuple[pd.DataFrame, str, int, int]:
    out_df, used_wrong_csv, filled_count = attach_wrong_answers(df, wrong_csv, in_csv)
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Print gold/wrong/pred/score details for the first N used rows.",
    )
    ap.add_argument(
    ...
    df, used_wrong_csv, filled_wrong_count, missing_wrong = maybe_attach_wrong_answers(df, args.wrong_csv, args.in_csv)
    ...
    drop_unknown_for_closed = should_drop_unknown_for_closed(args.dataset, model)
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Filter NC-correct rows"):
        ...
        if img_path is None or (inline_mask is None and (det_path is None or mask_path is None)):
            missing += 1
        ...
        if is_open_answer_type(row):
            ...
            if is_blank_text(wrong):
                ...
                continue
            closed_options = [gold, wrong]
            if not drop_unknown_for_closed:
                closed_options.append(UNKNOWN_ANSWER)
            nc_candidates = dedupe_candidate_options(closed_options)
            if len(nc_candidates) < 2:
                tracker.mark_done(row_key, {"status": "skip_invalid_candidates", "mode": nc_eval_mode})
                tracker.update(
                    seen=seen,
            ...
            nc_lp_unknown = float(nc_scores.get(UNKNOWN_ANSWER, float("nan"))) if UNKNOWN_ANSWER in nc_scores else None
            nc_correct = nc_pred == gold

        used += 1
        mode_counts[nc_eval_mode] = int(mode_counts.get(nc_eval_mode, 0)) + 1
        pred_counts[nc_pred or "<empty>"] += 1
        if args.debug_rows > 0 and used <= args.debug_rows:
            debug_payload = {
                "row_index": int(idx),
                "question": question,
                "gold": gold,
                "wrong": wrong,
                "pred": nc_pred,
                "correct": bool(nc_correct),
                "mode": nc_eval_mode,
                "candidates": nc_candidates,
                "scores": nc_scores,
            }
            print("DEBUG_ROW", json.dumps(debug_payload, ensure_ascii=False))
    ...
    if pred_counts:
        print(f"NC pred counts: {json.dumps(dict(pred_counts.most_common()), ensure_ascii=False)}")
