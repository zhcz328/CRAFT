...
def score_continuation_with_cache(
    ...
    past = normalize_past_key_values_for_model(model, prompt_cache["past_key_values"])
...
def resume_key_for_ablation_sample(
    ...
    return (
        ...
        f"{sample['wrong']}\t{position}\t{trace_mode}\t{mask_scope}"
    )
...
def build_samples(args, df, processor, tokenizer, model, device):
    ...
    for idx, row in tqdm(iterable, total=(len(df.head(args.max_examples)) if args.max_examples > 0 else len(df)), desc="Build samples"):
        ...
        try:
            nc_scores = score_answer_candidates_eval_style(
                model, processor, nc_image, nc_prompt, gold, wrong, unknown, args.trace_mode
            )
            ctx_scores = score_answer_candidates_eval_style(
                model, processor, ic_image, ctx_prompt, gold, wrong, unknown, args.trace_mode
            )
...
def rebuild_sample_images(sample: Dict[str, Any], max_image_side: int, mask_scale: float):
    ...
    ic_image, _ = build_masked_image(
        ...
        mask_rle=json.loads(sample["mask_rle"]) if str(sample.get("mask_rle", "")).strip() else None,
        mask_height=int(str(sample["mask_h"]).strip()) if str(sample.get("mask_h", "")).strip() else None,
        mask_width=int(str(sample["mask_w"]).strip()) if str(sample.get("mask_w", "")).strip() else None,
...
def main():
    ...
    try:
        for s in tqdm(samples, desc="Ablate selected heads"):
            ...
            if args.mask_scope == "all":
                ab_nc_scores = score_answer_candidates_eval_style(
                    model,
                    processor,
                    nc_image,
                    s["nc_prompt"],
            ...
            ab_ctx_scores = score_answer_candidates_eval_style(
                model,
                processor,
                ctx_image,
                s["ctx_prompt"],
    ...
    tracker.finish(out_json=args.out_json, n_samples=n, n_selected_heads=len(selected_pairs))
