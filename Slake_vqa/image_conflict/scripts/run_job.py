...
from dataset_config import dataset_save_root, normalize_dataset_name
...
def main() -> int:
    ...
    ap.add_argument("--dataset", default="slake_vqa", help="Dataset preset key or alias, e.g. slake_vqa or heal-medvqa.")
    ...
    env["DATASET"] = normalize_dataset_name(args.dataset)
    if args.resume:
        env["RESUME"] = "1"

    now = datetime.now().strftime("%Y%m%d_%H%M%S")
    label_part = f"_{sanitize_path_component(args.label)}" if args.label else ""
    save_root = dataset_save_root(ROOT_DIR, env["DATASET"])
    record_dir = save_root / "runs"
