...
def main():
    ...
    if tracker.is_done("split") and out_train.exists() and out_val.exists():
        print("Done (resume).")
        print(f"Train rows: {out_train}")
        print(f"Val rows: {out_val}")
        tracker.finish(out_train=str(out_train), out_val=str(out_val))
        return
    ...
    tracker.mark_done("split", {"train_rows": len(train_df), "val_rows": len(val_df)})
    tracker.finish(out_train=str(out_train), out_val=str(out_val), train_rows=len(train_df), val_rows=len(val_df))
