...
def main():
    ...
    def run_round(scan_layers: List[int], out_path: str, round_name: str):
        ...
        if stage == "coarse":
            ...
            for layer_idx, head_idx in tqdm(pending_heads, desc=f"Coarse {round_name}", unit="head"):
        ...
        if stage == "coarse_rerank":
            ...
            for layer_idx, head_idx in assigned_heads:
                ...
                if tracker.is_done(resume_key) and head_key in rerank_scores_map:
                    continue
                pending_heads.append(head_key)
            for layer_idx, head_idx in tqdm(pending_heads, desc=f"Rerank {round_name}", unit="head"):
                handles = install_head_mask_hooks(model, {layer_idx: [head_idx]}, keep_mode=args.keep_mode)
                try:
                    eff_red_sum = 0.0
                    base_change_sum = 0.0
        ...
        if stage == "refine":
            ...
            for layer_idx, head_idx in assigned_heads:
                ...
                if tracker.is_done(resume_key) and head_key in results_map:
                    continue
                pending_heads.append(head_key)
            for layer_idx, head_idx in tqdm(pending_heads, desc=f"Refine {round_name}", unit="head"):
        ...
        if stage == "full":
            ...
            for layer_idx, head_idx in tqdm(pending_coarse_heads, desc=f"Coarse {round_name}", unit="head"):
                handles = install_head_mask_hooks(model, {layer_idx: [head_idx]}, keep_mode=args.keep_mode)
                try:
                    eff_red_sum = 0.0
                    n = 0
            ...
            if args.coarse_topk and args.coarse_topk > 0:
                ...
                for layer_idx, head_idx in tqdm(pending_rerank_heads, desc=f"Rerank {round_name}", unit="head"):
            ...
            for layer_idx, head_idx in tqdm(pending_refine_heads, desc=f"Refine {round_name}", unit="head"):
                handles = install_head_mask_hooks(model, {int(layer_idx): [int(head_idx)]}, keep_mode=args.keep_mode)
                try:
                    eff_red_sum = 0.0
                    base_change_sum = 0.0
