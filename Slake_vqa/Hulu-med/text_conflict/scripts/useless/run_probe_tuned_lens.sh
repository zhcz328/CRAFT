...
MODEL_PATH="${MODEL_PATH:-/path/to/Hulu-Med-4B}"
...
PROBE_DATA_TRAIN="probe/data/train_${POSITION}.jsonl"
PROBE_DATA_VAL="probe/data/val_${POSITION}.jsonl"
PROBE_FEATURE_TRAIN="probe/features/train_${POSITION}.pt"
PROBE_FEATURE_VAL="probe/features/val_${POSITION}.pt"
PROBE_CONFLICT_OUT="probe/results/conflict_${PROBE_TYPE}_${POSITION}"
PROBE_FOLLOW_OUT="probe/results/follow_${PROBE_TYPE}_${POSITION}"

LENS_DATA_TRAIN="tuned_lens/data/train_${POSITION}.jsonl"
LENS_DATA_VAL="tuned_lens/data/val_${POSITION}.jsonl"
LENS_TRAIN_OUT="tuned_lens/results/train_${POSITION}"
LENS_CKPT="${LENS_TRAIN_OUT}/tuned_lens.pt"
LENS_TRAJ_OUT="tuned_lens/results/trajectory_${POSITION}"

mkdir -p probe/features probe/results tuned_lens/results
...
if [[ "$RUN_PREPARE_DATA" == "1" ]]; then
  if should_run_if_missing "probe/data/summary_${POSITION}.json"; then
    run_cmd $PYTHON_BIN probe/prepare_data.py "${prepare_args[@]}"
  fi
  if should_run_if_missing "tuned_lens/data/summary_${POSITION}.json"; then
    run_cmd $PYTHON_BIN tuned_lens/prepare_data.py "${prepare_args[@]}"
  fi
fi
