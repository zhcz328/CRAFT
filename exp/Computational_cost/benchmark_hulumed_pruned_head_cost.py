...
def build_sample_inputs(module, processor, sample: dict, position: str, max_image_side: int):
...
def _make_device_index(index_list):
    cache = {}

    def _get(device):
        if device not in cache:
            cache[device] = torch.tensor(index_list, dtype=torch.long, device=device)
        return cache[device]

    return _get


def install_query_head_pruning(module, model, selected_pairs):
    layer_to_remove = module.pack_layer_to_heads(selected_pairs)
    layer2attn, _ = module._find_self_attn_modules(model)
    patches = []

    for layer_idx, remove_heads in layer_to_remove.items():
        attn = layer2attn.get(layer_idx)
        if attn is None:
            continue
        ...
        remove_set = {int(h) for h in remove_heads}
        keep_q_heads = [h for h in range(num_heads) if h not in remove_set]
        if not keep_q_heads:
            raise RuntimeError(f"Layer {layer_idx} would remove all query heads, unsupported.")

        used_kv_heads = sorted({h // num_key_value_groups for h in keep_q_heads})
        kv_old_to_new = {old: new for new, old in enumerate(used_kv_heads)}
        q_to_kv_new = [kv_old_to_new[h // num_key_value_groups] for h in keep_q_heads]

        keep_q_row_idx = []
        for h in keep_q_heads:
            keep_q_row_idx.extend(range(h * head_dim, (h + 1) * head_dim))
        keep_kv_row_idx = []
        for h in used_kv_heads:
            keep_kv_row_idx.extend(range(h * head_dim, (h + 1) * head_dim))

        keep_o_col_idx = keep_q_row_idx[:]

        get_keep_q_row_idx = _make_device_index(keep_q_row_idx)
        get_keep_kv_row_idx = _make_device_index(keep_kv_row_idx)
        get_keep_o_col_idx = _make_device_index(keep_o_col_idx)
        get_q_to_kv_new = _make_device_index(q_to_kv_new)

        original_forward = attn.forward

        def pruned_forward(self, hidden_states, position_embeddings, attention_mask, past_key_values=None, cache_position=None, **kwargs):
            input_shape = hidden_states.shape[:-1]
            device = hidden_states.device

            q_weight = self.q_proj.weight.index_select(0, get_keep_q_row_idx(device))
            k_weight = self.k_proj.weight.index_select(0, get_keep_kv_row_idx(device))
            v_weight = self.v_proj.weight.index_select(0, get_keep_kv_row_idx(device))
            o_weight = self.o_proj.weight.index_select(1, get_keep_o_col_idx(device))

            query_states = F.linear(hidden_states, q_weight)
            query_states = query_states.view(*input_shape, len(keep_q_heads), head_dim)
            query_states = self.q_norm(query_states).transpose(1, 2)

            key_states = F.linear(hidden_states, k_weight)
            key_states = key_states.view(*input_shape, len(used_kv_heads), head_dim)
            key_states = self.k_norm(key_states).transpose(1, 2)

            value_states = F.linear(hidden_states, v_weight)
            value_states = value_states.view(*input_shape, len(used_kv_heads), head_dim).transpose(1, 2)
            ...
            key_for_q = key_states.index_select(1, get_q_to_kv_new(device))
            value_for_q = value_states.index_select(1, get_q_to_kv_new(device))

            attn_weights = torch.matmul(query_states, key_for_q.transpose(2, 3)) * self.scaling
            if attention_mask is not None:
                causal_mask = attention_mask[:, :, :, : key_for_q.shape[-2]]
                attn_weights = attn_weights + causal_mask

            attn_weights = torch.softmax(attn_weights, dim=-1, dtype=query_states.dtype)
            attn_output = torch.matmul(attn_weights, value_for_q)
        ...
        attn.forward = types.MethodType(pruned_forward, attn)
        patches.append(
            {
                "layer_idx": layer_idx,
                "module": attn,
                "original_forward": original_forward,
                "keep_q_heads": keep_q_heads,
                "used_kv_heads": used_kv_heads,
                "removed_q_heads": sorted(remove_set),
            }
        )
...
def remove_query_head_pruning(patches):
...
def write_markdown(out_path: Path, payload: dict):
    ...
    pruned = payload["pruned_ctx"]
    delta_time = pruned["timing"]["elapsed_ms_mean"] - normal["timing"]["elapsed_ms_mean"]
    delta_pct = 0.0
    if normal["timing"]["elapsed_ms_mean"] != 0:
        delta_pct = delta_time / normal["timing"]["elapsed_ms_mean"] * 100.0

    delta_flops = pruned["profile"]["reported_total_flops_giga"] - normal["profile"]["reported_total_flops_giga"]
    delta_flops_pct = 0.0
    if normal["profile"]["reported_total_flops_giga"] != 0:
        delta_flops_pct = delta_flops / normal["profile"]["reported_total_flops_giga"] * 100.0
    ...
    lines = [
        ...
        f"- True-pruned ctx mean latency: {pruned['timing']['elapsed_ms_mean']:.3f} ms",
        f"- Latency delta: {delta_time:.3f} ms ({delta_pct:.2f}%)",
        "",
        "## FLOPs",
        "",
        f"- Normal reported FLOPs: {normal['profile']['reported_total_flops_giga']:.6f} GFLOPs",
        f"- True-pruned reported FLOPs: {pruned['profile']['reported_total_flops_giga']:.6f} GFLOPs",
        f"- FLOPs delta: {delta_flops:.6f} GFLOPs ({delta_flops_pct:.4f}%)",
        "",
        "## Outputs",
        "",
        f"- Normal pred: {normal['timing']['result']['pred']}",
        f"- True-pruned pred: {pruned['timing']['result']['pred']}",
        f"- Normal follow_conflict: {normal['timing']['result']['metrics']['follow_conflict']:.6f}",
        f"- True-pruned follow_conflict: {pruned['timing']['result']['metrics']['follow_conflict']:.6f}",
        "",
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Hulu-Med normal vs true compute-skipping head-pruned inference cost.")
    ...
    parser.add_argument("--output_stem", default="hulumed_before_question_true_pruned_cost")
    ...
    patches = install_query_head_pruning(module, model, selected_pairs)
    try:
        pruned_timing = benchmark_scenario(
            module=module,
            model=model,
            tokenizer=tokenizer,
            ctx_inputs=sample_inputs["ctx_inputs"],
        ...
        pruned_profile = profile_scenario(
            module=module,
            model=model,
            tokenizer=tokenizer,
            ctx_inputs=sample_inputs["ctx_inputs"],
        ...
        remove_query_head_pruning(patches)
        ...
        "patched_layers": [
            {
                ...
                "removed_q_heads": patch["removed_q_heads"],
                "kept_q_head_count": len(patch["keep_q_heads"]),
                "used_kv_heads": patch["used_kv_heads"],
            }
            for patch in patches
        ],
        "normal_ctx": {
    ...
    print(json.dumps(
        {
            ...
            "pruned_mean_ms": pruned_timing["elapsed_ms_mean"],
            "normal_reported_gflops": normal_profile["reported_total_flops_giga"],
            "pruned_reported_gflops": pruned_profile["reported_total_flops_giga"],
        },
        indent=2,
        ensure_ascii=False,
    ))
